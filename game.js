...
startButton.addEventListener('click', function() {
    ...
    var downloadTimer = setInterval(function(){
        ...
        if(timeleft <= 0){
            ...
            alert(`You Can't Dance Loser!! (score = ${score})`)
            music.pause();
            music.currentTime = 0
            i=0
            j=0
    ...
    document.getElementById("main").src="./img/default.jpg"
    // command.play();
    music.play();
...
document.addEventListener('keydown', function(event) {
    ...
    if (event.code === 'ArrowRight') {
        document.getElementById("main").src="./img/right.jpg"
        punch.play()
    }
    if (event.code === 'ArrowDown') {
        document.getElementById("main").src="./img/down.jpg"
        punch.play()
    }
    if (event.code === 'ArrowLeft') {
        document.getElementById("main").src="./img/left.jpg"
        punch.play()
    }
    if (event.code === 'Space') {
        document.getElementById("main").src="./img/space.jpg"
        punch.play()
        applause.play()
        j++
    }
    ...
    if (j === answer.length) {
        alert (`You Can Dance!! Unbelievable!! (score = ${score+1})`)
        achievement.play()
        music.pause();
        music.currentTime = 0
        // command.pause();
